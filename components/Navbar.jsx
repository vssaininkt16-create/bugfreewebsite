  ...
  return (
    ...
    >
      <div className="container mx-auto px-4">
        <div className="flex justify-between items-center h-28">
          ...
          <div className="flex items-center flex-shrink-0 select-none">
            <img
              ...
              className="object-contain w-[150px] md:w-[280px] h-auto max-w-none"
              style={{
                pointerEvents: 'none',
          ...
          <div className="hidden md:flex items-center space-x-8">
            {navLinks.map((link) => (
              <Link
                key={link.name}
                href={link.href}
                className="relative text-gray-300 hover:text-cyber-green transition font-medium text-sm uppercase tracking-wider group"
              >
                {link.name}
            ...
            <Button className="bg-gradient-to-r from-cyber-green to-cyber-cyan text-black font-bold px-6 py-2 rounded-lg hover:shadow-lg hover:shadow-cyber-green/50 transition">
              Request Assessment
            </Button>
          </div>
        ...
        {/* Mobile Menu */}
        {isOpen && (
          <div className="md:hidden glass-card border-t border-cyber-green/20 py-4 mb-4 rounded-b-lg">
            {navLinks.map((link) => (
              <Link
                key={link.name}
                href={link.href}
